...
function read_city(filename::String)
    ...
    num_junctions, num_streets, total_time, fleet_size, start =
        parse.(Int, convert(Vector{String}, split(lines[1])))

    streets = []
    for i in 1:num_streets
        street_info =
            parse.(Int, convert(Vector{String}, split(lines[i + num_junctions + 1])))
        push!(
            streets,
            JBStreet(
                street_info[1] + 1,
